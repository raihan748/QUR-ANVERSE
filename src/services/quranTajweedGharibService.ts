 ...
 * Uses high-precision word tokenization and cross-word AST validation.
 */
  ...
  lines.forEach((line) => {
    if (line.type === 'text' && line.text) {
      ...
      words.forEach((w, wIdx) => {
        ...
        const prevW = words[wIdx - 1] || '';
        const isEnd = wIdx === words.length - 1;
        const res = getTajweedColorForWord(w, nextW, prevW, isEnd);

        if (res.ruleName) {
          const uniqueKey = `${res.ruleName}-${w}`;

          if (!seenRules.has(uniqueKey) && rulesList.length < 50) {
            seenRules.add(uniqueKey);

            let cat: TajwidRuleItem['ruleCategory'] = 'mad';
            const rLower = res.ruleName.toLowerCase();
            if (rLower.includes('ghunnah')) cat = 'ghunnah';
            else if (rLower.includes('qalqalah')) cat = 'qalqalah';
            else if (rLower.includes('syafawi') || rLower.includes('mimi')) cat = 'mim_mati';
            else if (rLower.includes('izhar halqi') || rLower.includes('idgham bighunnah') || rLower.includes('idgham bilaghunnah') || rLower.includes('ikhfa') || rLower.includes('iqlab') || rLower.includes('izhar muthlaq')) cat = 'nun_mati_tanwin';
            else if (rLower.includes('lam') || rLower.includes('qamariyah') || rLower.includes('syamsiyah') || rLower.includes('ra')) cat = 'lam_ra';

            // Find encyclopedia entry
            const enc = MASTER_TAJWEED_ENCYCLOPEDIA.find((e) => 
              res.ruleName && (
                e.title.toLowerCase().includes(res.ruleName.toLowerCase().split('(')[0].trim()) ||
                res.ruleName.toLowerCase().includes(e.title.toLowerCase().split('(')[0].trim())
              )
            );

            const pengertianBahasa = enc ? enc.pengertianBahasa : 'Kaidah fonetik bacaan Al-Qur\'an.';
            const pengertianIstilah = enc ? enc.pengertianIstilah : `Penerapan hukum ${res.ruleName} pada lafadz Al-Qur'an.`;
            const caraBaca = enc ? enc.caraBaca : 'Dibaca sesuai standar tartil tajwid.';
            ...
            rulesList.push({
              ...
              matchedWord: w,
              colorHex: res.color,
              harakatDuration: rLower.includes('6 harakat') ? 6 : (rLower.includes('4-5') ? 5 : (rLower.includes('dengung') ? 2 : 2)),
              pengertianBahasa,
              pengertianIstilah,
              description: enc?.summary || `Hukum bacaan ${res.ruleName} pada kata ${w}.`,
              caraBaca
            });
          }

          const baseRuleLabel = res.ruleName.split('(')[0].trim();
          ruleSummary[baseRuleLabel] = (ruleSummary[baseRuleLabel] || 0) + 1;
        }
      });
    }
  });
...
/**
 * Helper to strip diacritics and get the primary Arabic consonant of the following word.
 */
function getFirstConsonant(str?: string): string {
  if (!str) return '';
  const clean = str.replace(/[\u064B-\u065F\u0670\u06D6-\u06ED]/g, '');
  return clean[0] || '';
}

const HALQI_QATHI_LETTERS = ['ء', 'ه', 'ع', 'ح', 'غ', 'خ', 'أ', 'إ', 'آ', 'ؤ', 'ئ'];
 ...
 * High-accuracy AST regex matching with cross-word context to eliminate false positives.
 */
export function getTajweedColorForWord(
  word: string,
  nextWord: string = '',
  prevWord: string = '',
  isLineEnd: boolean = false
): { color: string; bg: string; ruleName?: string } {
  if (!word || word.length === 0) {
    return { color: '#064E3B', bg: 'transparent' };
  }

  const w = word.trim();
  const nextFirst = getFirstConsonant(nextWord);

  // 0. Bacaan Gharib & Khusus (Standar Riwayat Hafs 'an 'Ashim Thariq Asy-Syathibiyyah)
  if (w.includes('مَجْرٰ۪ىهَا')) {
    return { color: '#D97706', bg: '#FEF3C7', ruleName: 'Imalah (Suara E Miring - Hud: 41)' };
  }
  if (w.includes('تَأْمَ۫نَّا')) {
    return { color: '#D97706', bg: '#FEF3C7', ruleName: 'Isymam / Ikhtilas (Gerakan Bibir - Yusuf: 11)' };
  }
  if (w.includes('ءَ۬اعْجَمِىٌّ')) {
    return { color: '#D97706', bg: '#FEF3C7', ruleName: 'Tashil (Meringankan Hamzah - Fussilat: 44)' };
  }
  if (w.includes('بِئْسَ ٱلِٱسْمُ') || (w.includes('ٱلِٱسْمُ') && prevWord.includes('بِئْسَ'))) {
    return { color: '#D97706', bg: '#FEF3C7', ruleName: 'Naql (Pindah Harakat Kasrah - Al-Hujurat: 11)' };
  }
  if (/عِوَجَاۜ|مَّرْقَدِنَاۜ|مَنْۜ|بَلْۜ/.test(w) || w.includes('ۜ')) {
    return { color: '#D97706', bg: '#FEF3C7', ruleName: 'Saktah (Berhenti 2 Harakat Tanpa Nafas)' };
  }

  // Sifir Mustathil (Lonjong) vs Mustadir (Bulat)
  if (/أَنَا۟|لَّـٰكِنَّا۟|ٱلظُّنُونَا۟|ٱلرَّسُولَا۟|ٱلسَّبِيلَا۟|قَوَارِيرَا۟/.test(w)) {
    return { color: '#D97706', bg: '#FEF3C7', ruleName: 'Sifir Mustathil (Waqaf 2 Harakat, Washal Gugur)' };
  }
  if (/[\u06DF]/.test(w)) {
    return { color: '#6B7280', bg: '#F3F4F6', ruleName: 'Sifir Mustadir (Huruf Tambahan Gugur Mutlak)' };
  }

  // Izhar Muthlaq (4 kata khusus dalam 1 kalimat)
  if (/دُنْيَا|بُنْيَان|قِنْوَان|صِنْوَان/.test(w)) {
    return { color: '#059669', bg: '#D1FAE5', ruleName: 'Izhar Muthlaq (Wajib Jelas Tanpa Dengung)' };
  }

  // Idgham Mutajanisain & Mutaqaribain Khusus
  if (/بَسَطتَ|أَحَطتُ|فَرَّطتُ|فَرَّطتُمْ/.test(w)) {
    return { color: '#4338CA', bg: '#E0E7FF', ruleName: 'Idgham Mutajanisain Naqis (Tha melebur ke Ta, Ithbaq tetap)' };
  }
  if (/أَلَمْ نَخْلُقكُّم|نَّخْلُقكُّم/.test(w)) {
    return { color: '#4338CA', bg: '#E0E7FF', ruleName: 'Idgham Mutaqaribain (Qaf melebur ke Kaf)' };
  }
  if (w.includes('يَلْهَث') && nextWord.includes('ذَّٰلِكَ')) {
    return { color: '#4338CA', bg: '#E0E7FF', ruleName: 'Idgham Mutajanisain (Tha melebur ke Dzal)' };
  }
  if (w.includes('ٱرْكَب') && nextWord.includes('مَّعَنَا')) {
    return { color: '#4338CA', bg: '#E0E7FF', ruleName: 'Idgham Mutajanisain (Ba melebur ke Mim ber-ghunnah)' };
  }

  // 1. Mad Shilah Thawilah (ـهُۥٓ / ـهِۦٓ) & Shilah Qashirah (ـهُۥ / ـهِۦ)
  if (/[\u06E5\u06E6][\u0653~]/.test(w)) {
    return { color: '#B91C1C', bg: '#FEE2E2', ruleName: 'Mad Shilah Thawilah (4-5 Harakat)' };
  }
  if (/[\u06E5\u06E6]/.test(w)) {
    return { color: '#92400E', bg: '#FEF3C7', ruleName: 'Mad Shilah Qashirah (2 Harakat)' };
  }

  // 2. Mad Lazim (Mad bendera ~ bertemu tasydid atau huruf Muqatta'at: 6 harakat)
  if (/[\u0653~].*[\u0651]/.test(w) || /^(?:الٓمٓ|الٓمٓصٓ|الٓرٰ|الٓمٓرٰ|كٓهٰيٰعٓصٓ|طٰهٰ|طٰسٓمٓ|طٰسٓ|يٰسٓ|صٓ|حٰمٓ|عٓسٓقٓ|قٓ|نٓ)/.test(w)) {
    return { color: '#991B1B', bg: '#FEE2E2', ruleName: 'Mad Lazim (Wajib 6 Harakat)' };
  }

  // 3. Mad Wajib Muttashil (Mad bendera ~ diikuti Hamzah dalam SATU kata yang sama: 4-5 harakat)
  if (/[\u0653~][^ءأإؤئ\s]*[ءأإؤئ]/.test(w) || /[\u0653~][ءأإؤئ]/.test(w)) {
    return { color: '#B91C1C', bg: '#FEE2E2', ruleName: 'Mad Wajib Muttashil (4-5 Harakat)' };
  }

  // 4. Mad Jaiz Munfashil (Mad bendera ~ di akhir kata bertemu Hamzah di kata berikutnya: 4-5 harakat)
  if (/[\u0653~]/.test(w)) {
    return { color: '#C026D3', bg: '#FAE8FF', ruleName: 'Mad Jaiz Munfashil (4-5 Harakat)' };
  }

  // Preceding word contextual analysis
  const prevHasTanwin = /[ًٌٍ]/.test(prevWord);
  const prevIsVowelledNun = /ن[\u064E\u064F\u0650\u0651]$/.test(prevWord);
  const prevHasNunSukun = (/(?:ن[\u0652]?|ن)$/.test(prevWord)) && !prevIsVowelledNun;
  const isIncomingFromTanwinOrNun = prevHasTanwin || prevHasNunSukun;

  // 5. End of Ayah / Waqaf Special Rules (Mad 'Aridh Lissukun, Mad 'Iwad, Qalqalah Kubra)
  if (isLineEnd) {
    // Mad 'Iwad: HANYA terjadi pada Fathatain (ً / \u064B), TIDAK berlaku pada kasratain/dammatain
    if (/[\u064B]ا?$|[\u064B]ى$/.test(w)) {
      return { color: '#059669', bg: '#D1FAE5', ruleName: "Mad 'Iwad (Waqaf Fathatain 2 Harakat)" };
    }
    // Qalqalah Kubra: Huruf Qalqalah di akhir kata waqaf
    if (/[قطبجد][\u064B-\u065F\u0670\u0651\u06ED]?$/.test(w)) {
      return { color: '#DC2626', bg: '#FEE2E2', ruleName: 'Qalqalah Kubra (Pantulan Tebal Waqaf)' };
    }
    // Mad 'Aridh Lissukun: Huruf Mad (Ya, Wawu, Alif) atau Lin sebelum konsonan terakhir yang disukunkan karena waqaf
    if (/(?:[\u064E\u0670]?ا|[\u0650]?ي|[\u064F]?و|[\u064E]ي[\u0652]?|[\u064E]و[\u0652]?|[يىو])[نلمربتدذجحخسشعفقكطظضص]/u.test(w)) {
      return { color: '#2563EB', bg: '#DBEAFE', ruleName: "Mad 'Aridh Lissukun (Waqaf 2, 4, atau 6 Harakat)" };
    }
  }

  // 6. Outgoing Nun Sukun & Tanwin (Cross-Word Context to nextWord)
  const hasTanwinEnd = /[ًٌٍ]/.test(w);
  const isVowelledNunEnd = /ن[\u064E\u064F\u0650\u0651]$/.test(w);
  const hasNunSukunEnd = (/(?:ن[\u0652]?|ن)$/.test(w)) && !isVowelledNunEnd;

  if ((hasTanwinEnd || hasNunSukunEnd) && nextFirst) {
    if (hasTanwinEnd && (nextFirst === 'ٱ' || nextWord.startsWith('ٱل'))) {
      return { color: '#0891B2', bg: '#CFFAFE', ruleName: 'Nun Wiqayah / Nun Wasl (Tanwin dibaca Kasrah - Iltiqa Sakinain)' };
    }
    if (IDGHAM_BIGHUNNAH_LETTERS.includes(nextFirst)) {
      return { color: '#2563EB', bg: '#DBEAFE', ruleName: `Idgham Bighunnah (Melebur ke ${nextFirst} ber-ghunnah)` };
    }
    if (IDGHAM_BILAGHUNNAH_LETTERS.includes(nextFirst)) {
      return { color: '#4F46E5', bg: '#E0E7FF', ruleName: `Idgham Bilaghunnah (Melebur ke ${nextFirst} tanpa dengung)` };
    }
    if (nextFirst === 'ب' || w.includes('\u06E2')) {
      return { color: '#7C3AED', bg: '#EDE9FE', ruleName: 'Iqlab (Suara N berganti M mendengung)' };
    }
    if (HALQI_QATHI_LETTERS.includes(nextFirst)) {
      return { color: '#059669', bg: '#D1FAE5', ruleName: `Izhar Halqi (Jelas & Terang bertemu ${nextFirst})` };
    }
    if (IKHFA_HAQIQI_LETTERS.includes(nextFirst)) {
      return { color: '#DB2777', bg: '#FCE7F3', ruleName: `Ikhfa' Haqiqi (Samar mendengung menuju ${nextFirst})` };
    }
  }

  // 7. Intra-Word Nun Sukun & Tanwin Rules
  if (/(?:ن[\u0652]?)[تثجدذزسشصضطظفقك]/.test(w)) {
    return { color: '#DB2777', bg: '#FCE7F3', ruleName: "Ikhfa' Haqiqi (Samar Dengung 2 Harakat)" };
  }
  if (/(?:ن[\u0652]?)[ءهعحغخأإآؤئ]/.test(w)) {
  ...
  // 8. Incoming Assimilation (from previous word tanwin / nun sukun)
  if (isIncomingFromTanwinOrNun) {
    if (/^[ينمو]\u0651|^[\u064E\u064F\u0650]?[ينمو]\u0651|^[ينمو]/.test(w)) {
      return { color: '#2563EB', bg: '#DBEAFE', ruleName: 'Idgham Bighunnah (Tanwin/Nun melebur berdengung 2 harakat)' };
    }
    if (/^[لر]\u0651|^[\u064E\u064F\u0650]?[لر]\u0651|^[لر]/.test(w)) {
      return { color: '#4F46E5', bg: '#E0E7FF', ruleName: 'Idgham Bilaghunnah (Tanwin/Nun melebur tanpa dengung)' };
    }
    if (/^ب/.test(w) || prevWord.includes('\u06E2')) {
      return { color: '#7C3AED', bg: '#EDE9FE', ruleName: 'Iqlab (Suara Tanwin/Nun berganti M mendengung)' };
    }
    if (/^[تثجدذزسشصضطظفقك]/.test(w)) {
      return { color: '#DB2777', bg: '#FCE7F3', ruleName: "Ikhfa' Haqiqi (Samar mendengung 2 harakat)" };
    }
    if (/^[ءهعحغخأإآؤئ]/.test(w)) {
      return { color: '#059669', bg: '#D1FAE5', ruleName: 'Izhar Halqi (Dibaca jelas tanpa dengung)' };
    }
  }

  // 9. Ghunnah Musyaddadah ASHLIYYAH (Nun / Mim bertasydid murni bawaan kata)
  const isFirstLetterShaddah = /^[نم]\u0651|^[\u064E\u064F\u0650]?[نم]\u0651/.test(w);
  const isInternalGhunnahAshli = /(?:[^\s]ن\u0651|[^\s]م\u0651|ن[\u064E\u064F\u0650]\u0651|م[\u064E\u064F\u0650]\u0651)/.test(w);
  if (isInternalGhunnahAshli || (isFirstLetterShaddah && !isIncomingFromTanwinOrNun)) {
    return { color: '#047857', bg: '#D1FAE5', ruleName: 'Ghunnah Musyaddadah (Dengung Asli 2-3 Harakat)' };
  }

  // 10. Iqlab Internal (Nun sukun bertemu Ba dalam kata atau tanda meem kecil)
  const isIqlabInternal = /(?:[ًٌٍ]|ن[\u0652]?)\s*ب/.test(w) || /ن[\u06E2]ب|[\u06E2]ب/.test(w);
  const isIqlabCross = (/(?:[ًٌٍ]|ن[\u0652]?)$/.test(w) || /[\u06E2]$/.test(w)) && nextFirst === 'ب';
  if (isIqlabInternal || isIqlabCross) {
    return { color: '#7C3AED', bg: '#EDE9FE', ruleName: 'Iqlab (Suara N berganti M mendengung)' };
  }

  // 11. Mim Sukun (مْ) Rules
  const isVowelledMimEnd = /م[\u064E\u064F\u0650\u0651]$/.test(w);
  const hasMimSukunEnd = (/(?:م[\u0652]?|م)$/.test(w)) && !isVowelledMimEnd;

  if (hasMimSukunEnd && nextFirst) {
    if (nextFirst === 'ب') {
      return { color: '#C026D3', bg: '#FAE8FF', ruleName: "Ikhfa' Syafawi (Mim mati bertemu Ba)" };
    }
    if (nextFirst === 'م') {
      return { color: '#0D9488', bg: '#CCFBF1', ruleName: 'Idgham Mimi (Mim mati melebur ke Mim)' };
    }
    if (nextFirst === 'و' || nextFirst === 'ف') {
      return { color: '#0891B2', bg: '#CFFAFE', ruleName: `Izhar Syafawi Ashadd (Ekstra jelas bertemu ${nextFirst})` };
    }
    return { color: '#0891B2', bg: '#CFFAFE', ruleName: `Izhar Syafawi (Mim mati dibaca jelas bertemu ${nextFirst})` };
  }
  if (/م[\u0652]?\s*ب/.test(w)) {
  ...
  // 12. Qalqalah Sughra (Pantulan Huruf ق, ط, ب, ج, د yang bersukun di tengah kata)
  if (/[قطبجد][\u0652]/.test(w)) {
    return { color: '#C2410C', bg: '#FFEDD5', ruleName: 'Qalqalah Sughra (Pantulan Ringan)' };
  }

  // 13. Lam Jalalah (Lafadz Allah)
  if (/(?:ٱللَّ|اللَّ|لِلَّ|ٱللّٰ|اللّٰ)/.test(w)) {
    if (/^(?:بِ|لِ|فِى\s*)[\u0650]?/.test(w) || /لِلَّ|لِلّٰ|بِٱللَّ|بِٱللّٰ|بِاللَّ|بِاللّٰ/.test(w)) {
      return { color: '#0369A1', bg: 'transparent', ruleName: 'Lam Jalalah Tarqiq (Tipis - Lillah)' };
    }
    if (prevWord) {
      const cleanPrev = prevWord.replace(/[\u06D6-\u06ED]/g, '').trim();
      if (/[\u0650ٍي]$/.test(cleanPrev)) {
        return { color: '#0369A1', bg: 'transparent', ruleName: 'Lam Jalalah Tarqiq (Tipis - Lillah)' };
      }
    }
  ...
  // 14. Alif Lam Syamsiyah & Qamariyah
  ...
  // 15. Mad Thabi'i (Asli 2 Harakat)
  if (/(?:[\u064E\u0670]ا|[\u0650]ي|[\u064F]و|[\u0670])/.test(w)) {
    return { color: '#047857', bg: 'transparent', ruleName: "Mad Thabi'i (Mad Asli 2 Harakat)" };
  }
