import React from 'react';
import { 
  BookOpen, 
  Mic2, 
  EyeOff, 
  Swords, 
  Compass, 
  LayoutDashboard, 
  DownloadCloud, 
  Clock, 
  MapPin,
  Sparkles,
  Zap,
  Cpu,
  ScrollText,
  SunMoon
} from 'lucide-react';
import { NavigationTab, PrayerTime } from '../../types';
import { NeobrutalCard } from './NeobrutalCard';
import { useLanguage } from '../../context/LanguageContext';

interface SidebarProps {
  activeTab: NavigationTab;
  onSelectTab: (tab: NavigationTab) => void;
  nextPrayer: PrayerTime | null;
  countdownStr: string;
  cityName?: string;
}

export const Sidebar: React.FC<SidebarProps> = ({
  activeTab,
  onSelectTab,
  nextPrayer,
  countdownStr,
  cityName
}) => {
  const { language, t, isRtl } = useLanguage();

  const navItems = [
    {
      id: 'mushaf' as NavigationTab,
      label: t.nav_mushaf,
      sublabel: t.nav_mushafSub,
      icon: BookOpen,
      badge: language === 'ar' ? '٣٠ جزء' : '30 Juz',
      color: 'bg-[#0B4627]',
      isPrimary: true
    },
    {
      id: 'tilawah' as NavigationTab,
      label: t.nav_tilawah,
      sublabel: t.nav_tilawahSub,
      icon: Sparkles,
      badge: language === 'ar' ? 'تشغيل تلقائي' : 'Auto Putar',
      color: 'bg-[#059669]',
      isPrimary: true
    },
    {
      id: 'murojaah_ai' as NavigationTab,
      label: t.nav_murojaah_ai,
      sublabel: t.nav_murojaah_aiSub,
      icon: Mic2,
      badge: language === 'ar' ? 'ذكاء اصطناعي' : 'AI Cerdas',
      color: 'bg-[#D97706]',
      isPrimary: true
    },
    {
      id: 'simai' as NavigationTab,
      label: t.nav_simai,
      sublabel: t.nav_simaiSub,
      icon: EyeOff,
      badge: language === 'ar' ? '٣ مستويات' : '3 Level',
      color: 'bg-[#4B5563]'
    },
    {
      id: 'challenge' as NavigationTab,
      label: t.nav_challenge,
      sublabel: t.nav_challengeSub,
      icon: Swords,
      badge: language === 'ar' ? 'تحدي الحفظ' : 'Game Mutqin',
      color: 'bg-[#9333EA]'
    },
    {
      id: 'prayer' as NavigationTab,
      label: t.nav_prayer,
      sublabel: t.nav_prayerSub,
      icon: Compass,
      badge: language === 'ar' ? 'أذان تلقائي' : 'Auto Adzan',
      color: 'bg-[#059669]'
    },
    {
      id: 'dzikir' as NavigationTab,
      label: t.nav_dzikir,
      sublabel: t.nav_dzikirSub,
      icon: SunMoon,
      badge: language === 'ar' ? 'صوت + تسبيح' : 'Audio + Tasbih',
      color: 'bg-[#0B4627]',
      isPrimary: true
    },
    {
      id: 'dashboard' as NavigationTab,
      label: t.nav_dashboard,
      sublabel: t.nav_dashboardSub,
      icon: LayoutDashboard,
      color: 'bg-[#2563EB]'
    },
    {
      id: 'download' as NavigationTab,
      label: t.nav_download,
      sublabel: t.nav_downloadSub,
      icon: DownloadCloud,
      badge: language === 'ar' ? 'بدون شبكة' : 'Offline',
      color: 'bg-[#0B4627]'
    },
    {
      id: 'asbabun_nuzul' as NavigationTab,
      label: language === 'ar' ? 'أسباب النزول وتاريخ الوحي' : 'Asbabun Nuzul',
      sublabel: language === 'ar' ? 'سياق الآيات وتاريخ التنزيل' : 'Histori & Konteks Wahyu',
      icon: ScrollText,
      badge: language === 'ar' ? 'تاريخي' : 'Konteks',
      color: 'bg-[#D97706]'
    }
  ];

  return (
    <aside className="w-64 bg-white/90 border-r border-slate-200/80 p-3.5 flex flex-col justify-between hidden lg:flex shrink-0 min-h-[calc(100vh-62px)]">
      <div className="space-y-3">
        {/* Section Title */}
        <div className="px-2 py-1 flex items-center justify-between">
          <span className="text-[11px] font-bold tracking-wider text-slate-500 uppercase">
            {language === 'ar' ? 'القائمة الرئيسية' : 'MENU UTAMA'}
          </span>
          <span className="text-[10px] font-medium text-emerald-800 bg-emerald-50 px-2 py-0.5 border border-emerald-200 rounded-md">
            {t.standardBadge}
          </span>
        </div>

        {/* Navigation List with Clean Minimalist Transitions */}
        <nav className="space-y-1">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;

            return (
              <button
                key={item.id}
                onClick={() => onSelectTab(item.id)}
                className={`w-full px-3 py-2.5 rounded-xl text-left flex items-center justify-between transition cursor-pointer ${
                  isActive
                    ? 'bg-[#0B4627] text-white font-semibold shadow-xs'
                    : 'text-slate-700 hover:text-slate-950 hover:bg-slate-100/80 font-medium'
                }`}
              >
                <div className="flex items-center gap-2.5">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                      isActive ? 'bg-amber-500 text-slate-950 font-bold' : 'bg-slate-100 text-slate-600'
                    }`}
                  >
                    <Icon className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-xs leading-tight font-semibold">{item.label}</p>
                  </div>
                </div>

                {item.badge && (
                  <span
                    className={`px-1.5 py-0.5 text-[9px] font-medium rounded ${
                      isActive ? 'bg-amber-400 text-slate-950 font-bold' : 'bg-slate-100 text-slate-600'
                    }`}
                  >
                    {item.badge}
                  </span>
                )}
              </button>
            );
          })}
        </nav>
      </div>

      {/* Makassar Prayer Widget in Desktop Sidebar */}
      <div className="mt-6 pt-4 border-t border-slate-200/80">
        <div className="p-3.5 bg-[#0B4627] text-white rounded-2xl border border-emerald-800/80 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-amber-300">
              <Clock className="w-3.5 h-3.5" />
              <span>Menuju {nextPrayer?.name || 'Shalat'}</span>
            </div>
            <span className="text-[10px] bg-black/30 text-emerald-200 px-1.5 py-0.5 rounded font-mono">
              {cityName || 'Makassar'}
            </span>
          </div>

          <div className="text-xl font-bold font-mono tracking-wider text-white">
            {countdownStr}
          </div>

          <div className="flex items-center gap-1 text-[10px] text-emerald-200/80">
            <MapPin className="w-3 h-3 text-amber-400" />
            <span>Waktu Shalat Otomatis & Adzan</span>
          </div>
        </div>
      </div>
    </aside>
  );
};
