...
export const PhysicalMushafPageReader: React.FC = () => {
      ...
      >
        ...
        <div className="w-full flex items-center justify-between pb-2 px-1 text-xs font-bold text-[#541E0A]">
          ...
          <div className="flex-1 flex items-center justify-start">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#FFFDF9] border border-[#8C3A1E]/80 rounded-full shadow-xs">
          ...
          <div className="flex-1 flex items-center justify-end">
            <div className="inline-flex items-center gap-1 px-3 py-1 bg-[#B91C1C] text-white border border-[#541E0A] rounded-full shadow-xs">
        ...
        <div className="w-full flex items-center justify-between pt-2 px-1 text-xs font-mono font-black text-[#541E0A]">
          ...
          <div className="px-2 text-center">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#FFFDF9] border border-[#8C3A1E]/80 rounded-full shadow-xs">
              <span className="font-sans text-xs font-bold text-[#7C2D12]">{pageNum}</span>
              <span className="text-[#8C3A1E]/60">|</span>
              <span className="font-quran text-sm font-bold text-[#7C2D12]">{toArabicNumerals(pageNum)}</span>
            </div>
          </div>
  ...
  return (
    ...
    >
      {/* FLOATING TOAST */}
      {toastMessage && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 bg-emerald-900 text-white px-4 py-2.5 rounded-2xl border border-emerald-700/80 shadow-lg text-xs font-semibold flex items-center gap-2 animate-bounce">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-4 sm:p-5 shadow-xs space-y-3.5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 bg-amber-500/20 text-amber-500 border border-amber-400/40 rounded-xl flex items-center justify-center font-bold shadow-xs">
            ...
            <div>
              <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                ...
                <span className="text-[10px] bg-[#0B4627] text-amber-300 px-2 py-0.5 rounded-md font-mono font-bold border border-emerald-700/60">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold shadow-xs">
              <button
            ...
            {viewMode === 'layout' && (
              <button
                ...
                className={`px-2.5 py-1.5 border rounded-xl text-xs font-semibold flex items-center gap-1.5 cursor-pointer shadow-xs transition-all ${
                  showWaqafIbtidaGuides 
                    ? 'bg-amber-100 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border-amber-300 dark:border-amber-700 font-bold' 
            ...
            <div className="relative">
              <button
                ...
                className="px-2.5 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 cursor-pointer shadow-xs transition-colors"
              ...
              {isReciterMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-80 max-h-96 overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 p-2 space-y-1">
                  <div className="px-2 py-1.5 border-b border-slate-100 dark:border-slate-800 font-bold text-xs text-slate-700 dark:text-slate-300 flex justify-between items-center">
                    <span>Pilih Qari Tilawah:</span>
                    <span className="text-[10px] bg-amber-100 dark:bg-amber-950/60 px-1.5 py-0.5 rounded font-semibold text-amber-900 dark:text-amber-300 border border-amber-200 dark:border-amber-800">{RECITERS_LIST.length} Qari</span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="truncate pr-2">
                            ...
                            <p className={`text-[10px] ${isSelected ? 'text-amber-300 font-medium' : 'text-slate-400'}`}>
                              {r.style}
                            </p>
                          </div>
                        </button>
            ...
            <button
              ...
              className={`p-1.5 border rounded-xl cursor-pointer shadow-xs transition-all ${
                pageSoundEnabled 
            ...
            >
              {pageSoundEnabled ? <Volume2 className="w-4 h-4 text-emerald-700 dark:text-emerald-400" /> : <VolumeX className="w-4 h-4" />}
            </button>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className={`px-3 py-1.5 border rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-xs transition-all ${
                  isAudioActive
                    ? isAudioPaused
              ...
              >
                {isAudioActive ? (
                  isAudioPaused ? <Play className="w-3.5 h-3.5 fill-slate-950" /> : <Pause className="w-3.5 h-3.5 fill-slate-950" />
                ) : (
              ...
              {isAudioActive && (
                <button
                  ...
                  className="px-2.5 py-1.5 bg-red-600 hover:bg-red-700 text-white border border-red-500 rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer shadow-xs transition-colors"
            ...
            <button
              ...
              className="p-1.5 bg-white dark:bg-slate-800 hover:bg-amber-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer shadow-xs transition-colors"
            ...
            <button
              ...
              className="p-1.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer shadow-xs transition-colors"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
          <div className="sm:col-span-3 flex items-center gap-1.5">
            <span className="font-semibold text-slate-600 dark:text-slate-400 shrink-0">Juz:</span>
            <select
              value={juzNumberRight}
              onChange={(e) => handleJumpToJuz(Number(e.target.value))}
              className="w-full bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-xl px-2.5 py-1.5 font-medium cursor-pointer shadow-xs focus:outline-none focus:ring-2 focus:ring-[#0B4627]"
            >
          ...
          <div className="sm:col-span-5 flex items-center gap-1.5">
            <span className="font-semibold text-slate-600 dark:text-slate-400 shrink-0">Surat:</span>
            <select
          ...
          <div className="sm:col-span-4 flex items-center gap-1.5">
            <span className="font-semibold text-slate-600 dark:text-slate-400 shrink-0">Hal:</span>
            <input
              type="number"
              min={1}
              max={604}
              ...
              className="w-20 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-xl px-2 py-1.5 font-bold text-center shadow-xs focus:outline-none focus:ring-2 focus:ring-[#0B4627]"
            />
            <span className="font-medium text-slate-400">/ 604</span>
          </div>
        </div>
      </div>
      ...
      <div 
        className={`relative bg-[#EADBBE] dark:bg-[#0F172A] border border-amber-950/40 rounded-2xl p-2 sm:p-5 shadow-xl overflow-hidden ${
      ...
      >
        ...
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 z-30 p-2 sm:p-3 bg-[#FFFDF7] hover:bg-[#FEF3C7] disabled:opacity-30 text-slate-900 border border-amber-900/30 rounded-full shadow-md hover:shadow-lg cursor-pointer transition-all"
        ...
        <button
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 z-30 p-2 sm:p-3 bg-[#FFFDF7] hover:bg-[#FEF3C7] disabled:opacity-30 text-slate-900 border border-amber-900/30 rounded-full shadow-md hover:shadow-lg cursor-pointer transition-all"
        ...
        {isAudioActive && currentPlayingVerse && (
          <div className="absolute bottom-3 left-4 right-4 sm:left-8 sm:right-8 bg-gradient-to-r from-[#0B4627] via-[#064E3B] to-[#0B4627] text-white p-3 sm:p-4 rounded-2xl border border-amber-400/70 shadow-2xl flex flex-wrap items-center justify-between gap-3 z-40 animate-fade-in backdrop-blur-md">
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                onClick={handleTogglePlayPause}
                className={`px-3 py-2 font-semibold rounded-xl border border-slate-200/40 flex items-center gap-1.5 text-xs shadow-xs cursor-pointer transition-colors ${
                  isAudioPaused 
              ...
              <button 
                onClick={handleStopPageAudio}
                className="px-3 py-2 bg-rose-600 hover:bg-rose-700 text-white font-semibold rounded-xl border border-rose-500/40 flex items-center gap-1.5 text-xs shadow-xs cursor-pointer transition-colors"
                title="Hentikan Audio Sepenuhnya"
              >
                <Square className="w-3.5 h-3.5 fill-white" />
                <span>Stop</span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold flex-wrap gap-1">
            <button
            ...
            {pageTajweedData?.rulesList && pageTajweedData.rulesList.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                {pageTajweedData.rulesList.map((t, idx) => (
                  <div 
                    key={idx}
                    className="p-3 bg-white dark:bg-slate-800 border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xs flex flex-col justify-between space-y-1.5"
                  >
                    <div className="flex items-center justify-between">
                      <span 
                    ...
                    <div className="pt-1 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between text-[11px]">
                      <span className="font-medium text-slate-400">Durasi:</span>
                      <span className="font-semibold text-emerald-800 dark:text-emerald-300">{t.harakatDuration} Harakat</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {pageGharibData && pageGharibData.length > 0 ? (
              <div className="space-y-3">
                {pageGharibData.map((gharib, gIdx) => (
                  <div key={gIdx} className="p-4 bg-amber-50/80 dark:bg-amber-950/30 border border-amber-300/80 dark:border-amber-800 rounded-2xl space-y-2">
                    <div className="flex items-center gap-2 text-amber-900 dark:text-amber-300">
                      <AlertCircle className="w-5 h-5 text-amber-600" />
                      <h5 className="font-bold text-sm">{gharib.title} ({gharib.surahName} : {gharib.ayahNumber})</h5>
                    </div>
                    <p className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed font-normal">
                      {gharib.description}
                    </p>
                    <div className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-amber-300/50 dark:border-amber-700/40 font-mono text-xs">
                      <span className="font-semibold text-emerald-800 dark:text-emerald-300">Cara Membaca: </span>
                      <span className="text-slate-700 dark:text-slate-200">{gharib.caraBaca}</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-xs font-medium text-slate-500 dark:text-slate-400 space-y-1">
                <p>Tidak ada bacaan Gharib khusus pada halaman {rightPageNumber}.</p>
                <p className="text-[11px] text-slate-400">Gharib terdapat pada ayat-ayat tertentu seperti Imalah (Hud: 41), Isymam (Yusuf: 11), Tashil (Fushshilat: 44), Naql (Al-Hujurat: 11), dan Saktah.</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTajweedTab === 'legend' && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
            <div className="p-3 bg-red-50/70 dark:bg-red-950/30 border border-red-200 dark:border-red-900/60 rounded-2xl">
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1">Mad Wajib, Jaiz, Lazim, 'Aridh (2-6 harakat)</p>
            </div>
            <div className="p-3 bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/60 rounded-2xl">
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1">Dengung sempurna 2-3 harakat</p>
            </div>
            <div className="p-3 bg-sky-50/70 dark:bg-sky-950/30 border border-sky-200 dark:border-sky-900/60 rounded-2xl">
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1">Samar-samar ber-ghunnah 2 harakat</p>
            </div>
            <div className="p-3 bg-purple-50/70 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-900/60 rounded-2xl">
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1">Menukar suara N ke M dengan dengung</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-[#0B4627]"
                />
              </div>
              <div className="flex gap-1 overflow-x-auto pb-1 text-xs">
                {['Semua', 'Nun & Tanwin', 'Mim Sukun', 'Ghunnah & Qalqalah', 'Mad Lengkap', 'Bacaan Gharib', 'Tanda Waqaf'].map((cat) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl font-medium whitespace-nowrap border cursor-pointer transition-all ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-96 overflow-y-auto pr-1">
              {filteredEncyclopedia.map((entry) => (
                <div 
                  key={entry.id}
                  className="p-3.5 bg-white dark:bg-slate-800 border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xs space-y-2"
                >
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700/60 pb-2">
                    <div>
                      <h6 className="font-bold text-xs text-slate-900 dark:text-white">{entry.title}</h6>
                      <span className="font-quran text-xs text-amber-800 dark:text-amber-400 font-bold">{entry.arabicName}</span>
                    </div>
                    <span 
